...
from omgfm.ast_tools import walk, to_c_string, from_c_string, make_html_node, replace_node
...
class TOCGenerator(Processor):
    ...
    def process_input(self, root_node, max_depth=6, inject=True):
        ...
        html = self.toc_to_html(nested)
        if self.inject:
            toc = make_html_node(block=True, content=html)
            _cmark.node_prepend_child(root_node, toc)
        self.data = html

    def set_heading_attributes(self, root_node, max_depth):
        """Iterate over root node, create heading tokens to generate HTML, replace heading nodes in tree with html.
        """
        ...
        for node, entering, node_type in walk(root_node):
            if not entering and node_type == _cmark.NODE_HEADING:
                ...
                if self.inject:
                    _id = '{}-{}'.format(line_number, text)
                    heading = template.format(level=level, _id =_id, text=text)
                    new_node = _cmark.node_new(_cmark.NODE_HTML_BLOCK)
                    _cmark.node_set_literal(new_node, to_c_string(heading))
    ...
    def toc_to_html(self, toc):
        """Create an HTML toc from tokens generated by :py:func:`~TOCGenerator.nest_toc_tokens`
        """
        base_toc = '<div class="toc">{items}</div>'
        base_list = '<ul>{items}</ul>'
        base_li = """
            <li class="toc-item" class="toc-link">
              <a class="toc-link" href="{href}" data-line-number="{line_number}">{text}{children}</a>
            </li>"""

        def build_toc_html(toc, list_temlpate):
            items=[]
            for item in toc:
                children = ''
                line_number = item.get('line_number')
                text = item.get('text')
                href = '#{}-{}'.format(line_number, text)
                if item.get('children') is not None:
                    children = build_toc_html(item['children'], base_list)
                
                li = base_li.format(
                         href=href, line_number=line_number, 
                         text=text, children=children
                         )
                items.append(li)
            return list_temlpate.format(items=''.join(items))
        
        items =  build_toc_html(toc, base_list)
        return base_toc.format(items=''.join(items))
